...
class Debugger(object):
  ...
  def save_kitti_format(self, results, img_path,opt,img_id='default',is_faster=False, counter=0):
    # points=points_dim[:16]
    # dim = points_dim[16:19]
    # points = np.array(points, dtype=np.int32).reshape(self.num_joints, 2)
    result_dir=os.path.join(os.getcwd(),'./data/results_for_bev/')
    file_number='0' * (6 - len(str(counter))) + str(counter)
    box=results[:4]
    if is_faster:
